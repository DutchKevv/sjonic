...
router.post('/', async (req: any, res, next) => {
	try {
		res.send(await userController.create(req.user, req.fields));
	} catch (error) {
		next(error);
	}
});
...
router.put('/password', async (req: any, res, next) => {
	try {
		res.send(await userController.updatePassword(req.user, req.fields.token, req.fields.password));
	} catch (error) {
		next(error);
	}
});
...
router.put('/:id', async (req: any, res, next) => {
	try {
		res.send(await userController.update(req.user, req.params.id, req.fields));
	} catch (error) {
		next(error);
	}
});
